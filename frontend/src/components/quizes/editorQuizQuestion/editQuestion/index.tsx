...
const EditQuestion = forwardRef(function (props: EditorQuizQuestionProps, ref) {
    ...
    const [tfCorrect, updateCorrect] = useState(
        props.question.correctAnswers![0]
    );
    ...
    useImperativeHandle(ref, () => ({
        getQuestion: () => {
            const newChoices = choicesRef.current
                .map((choiceRef) => choiceRef.getChoiceContent())
                .filter((choice) => choice.replaceAll(' ', '').length > 0);

            const correctAnswers = choicesRef.current
                .map((choiceRef) => choiceRef.getIsCorrect())
                .map((isCorrect, index) =>
                    isCorrect ? newChoices[index] : undefined
                )
                .filter((isCorrect) => isCorrect !== undefined);
        ...
        <div className={ classes.container }>
            ...
            { props.question.questionType === QuestionType.MC && (
                <>
                    <div className={ classes.choices }>
                        { choices.map((choice, index) => (
                            <EditChoice
                                ...
                                choiceIndex={ index }
                                choice={ choice }
                                isCorrect={ props.question.correctAnswers!.includes(
                                    choice
                                ) }
                                delete={ deleteChoice }
                                ref={ (ref: EditChoiceRef) => {
                                    if (choicesRef.current.length <= index)
                                        choicesRef.current.push(ref);
                    ...
                    <div>
                        <Button onClick={ addChoice } light={ true }>
                            Add Answer
                        </Button>
                    </div>
                </>
            ...
            { props.question.questionType === QuestionType.TF && (
                <div className={ classes.tfChoices }>
                    ...
                    <EditChoice
                        ...
                        delete={ deleteChoice }
                        ref={ (ref: EditChoiceRef) => {
                            if (choicesRef.current.length <= 1)
                                choicesRef.current.push(ref);
