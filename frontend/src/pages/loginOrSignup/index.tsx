...
export default function LoginOrSignup(props: { isLogin: boolean }) {
    ...
    async function signup() {
        ...
        if (u.replaceAll(' ', '').length !== u.length)
            return throwError('Username cannot contains spaces');
        else if (u.length > 255)
            return throwError(
            ...
            return throwError('Please enter a valid email address');
        else if (p.replaceAll(' ', '').length !== p.length)
            return throwError('Password cannot contain spaces');
