...
export default function SessionDashboard() {
    ...
    const [_answers, updateAnswers] = useState<{ [k: string]: Answer[] }[]>();
    ...
    useEffect(() => {
        (async function () {
            ...
            const res = await fetch(
                `${ SERVER_URL }/answers/getfromsession/${ sessionId }/`
            );
            ...
            const keys = Array.from(
                new Set(filteredAnswers.map((answer) => answer.studentName))
            );
            const compositeArray = keys.map((k) => ({
