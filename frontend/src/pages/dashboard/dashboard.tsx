...
export default function Dashboard() {
    ...
    const fetchQuizzes = useCallback(async function(): Promise<string | Quiz[]> {
        updateLoading(true);
    ...
    const fetchSessions = useCallback(async function(quizzes: Quiz[]) {
        ...
        const promises = await Promise.all(quizzes.map(
            q => fetch(`${ SERVER_URL }/sessions/all/${ q.id }/`)
        ));
    ...
    async function deleteQuiz(id: string) {
        throwConfirm('Are you sure you want to delete this quiz?', async (isConfirmed: boolean) => {
    ...
    async function stopSession(id: string) {
        ...
        const res = await fetch(`${ SERVER_URL }/sessions/terminate/${ sessionId }/`, {
            method: 'POST'
        });
    ...
    return (
        <div className={ classes.dashboardContainer }>
            ...
            <div className={ classes.classesContainer }>
                {
                    quizzes.filter(
                        q => q.name.toLowerCase().includes(filter.toLowerCase())
                    ).map(
                        (q, i) => (
                            <QuizCard
                                key={ i }
                                ...
                                session={ sessions.filter(s => s.sessionFor === q.id)[0] }
